...
/**
   ...
   @param {boolean} log Enable aq2rdb server logging if true; no
          logging if false.
*/
NWISRA: function (host, userName, password, log) {
    var authentication;
    var parameters;
     ...
     */
    function queryRemote(path, obj, log) {
        return rest.query(
            "https", host, "GET",
    ...
    /**
       ...
       @param {boolean} log Enable console logging if true; no console
                        logging when false.
    */
    this.query = function (obj, log) {
        return new Promise((resolve, reject) => {
            // if NWIS-RA is logged-into
            if (authentication) {
                queryRemote(
                    "/service/data/view/parameters/json", obj, log
                )
                    .then((messageBody) => {resolve(messageBody);});
            }
            else {
